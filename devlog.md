...
## Log 9: Nov 16 1:25pm
### I couldn't fix the problem without significantly changing anything so I decided to redo the whole queue system, and that included even making significant changes to the teller method in the beginning of it. The customer method remained unchanged. I decided to handle the queuing from the teller method itself becuase it anyways communicated with the customer, so having the queing management in the method allows the teller to know what is in the queue better so that it knows to serve another customer. I decided to run it in a while loop, which also runs the whole teller method. Essentially, the teller method is the man of the show rather than the main method, which would create and end threads and close the bank. This makes sense becuase nothing in the bank would start and finish without the tellers operating. I'm going to finish this up and see if it works, and to make it easier for myself, I might add some labels for some of the some to easy keep track of what I'm doing in the program
